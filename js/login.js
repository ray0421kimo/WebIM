		...
		Q1.ajax({
			...
			success: function(error, response) {
				...
				if (error) {
					...
					setCookie("_custid", response.data.custid);
					setCookie("_webtoken", response.data.webauth);
					setCookie("_token", response.data.token);
					setCookie("uid", response.data.custid);
					var my = JSON.stringify(response.data);
					setCookie("userJson", my);

					var currentUserId = window.localStorage.getItem("currentUserId");
					if (currentUserId != response.data.custid) {

						window.localStorage.clear();
					}

					window.localStorage.setItem("currentUserId", response.data.custid);
