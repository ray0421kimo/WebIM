    ...
    addEvent: function () {
       ...
       // this.$chooseFileBtn.on('click', 'a', this.chooseFile.bind(this));
        ...
        this.uploader = WebUploader.create({
            ...
            // 文件接收服务端。
            server: Q1.API_BASE_URL+'Chat/Chat/sendFile?_token='+_token+"&_custid="+_custid,

            // 选择文件的按钮。可选。
            // 内部根据当前运行是创建，可能是input元素，也可能是flash.
            pick: '#j-msgType',
            fileVal: "uploadedFile",
            // 只允许选择图片文件。
            //accept: {
            //    title: 'Images',
        ...
        this.uploader.on( 'fileQueued', function( file ) {
            ...
            that.mysdk.willSendFile({file:file});
        });

        this.uploader.on( 'uploadProgress', function( file, percentage ) {

            that.mysdk.sendFileProgress({file:file,percentage:percentage});

        });

        this.uploader.on( 'uploadSuccess', function( file,response ) {

            var scene = that.$chatEditor.data('type');
             var to = that.$chatEditor.data('to');

            var fileObj = {fileName:file.name,fileid:response.data.fileid,filehash:response.data.filehash,fileSize:file.size};
            that.mysdk.sendFileSuccess(scene,to,file,fileObj,that.sendMsgDone.bind(that));

        });
        this.uploader.on( 'uploadError', function( file ) {

            that.mysdk.sendFileError({file:file});
        });
