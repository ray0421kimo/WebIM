...
var SDKBridge = function(ctr, data) {
	...
	Q1.ajax({
		...
		success: function(error, response) {
			...
			var host = response.data.chat.host;
			var port = response.data.chat.port;

			initIM.bind(that)(host);

		}
	});

	var initIM = function(host) {
		window.nim = this.nim = new ITIM({
			serverURI: "ws://" + host + ":30000/ws",
			//控制台日志，上线时应该关掉
			debug: true || {
				api: 'info',
 ...
 * @param callback：回调
 */
SDKBridge.prototype.willSendFile = function(option) {

	console.log("willSendFile");
}

SDKBridge.prototype.sendFileProgress = function(option) {
	console.log("sendFileProgress==" + option * 100);
}

SDKBridge.prototype.sendFileSuccess = function(scene, to, file, fileObj, callback) {

	this.nim.sendFileSuccess({
		scene: scene,
		to: to,
		fileObj: fileObj,
		done: callback
...
SDKBridge.prototype.sendFileError = function(option) {
